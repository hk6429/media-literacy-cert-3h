...
獨立的教師線上增能與課程完成證書網站。教師只需第一次填寫姓名、學校與 Email；系統會保存三個模組、測驗、有效學習時間、微教案與證書資料。管理端可查詢完成名單及匯出 CSV。

## 功能

- 三小時課程：媒體建構、資訊查證、AI 深偽與教學轉化。
- 三個模組形成性評量，80 分通過。
- 12 題總測驗，80 分通過。
- 一頁媒體素養微教案。
- 頁面可見且持續互動時累積有效學習時間。
...
```text
src/                 Worker、課程題庫、Durable Object
public/              教師端、管理端、證書驗證與樣式
