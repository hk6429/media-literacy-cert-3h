...
describe("HTTP API", () => {
  it("提供不含答案的課程資料", async () => {
    ...
    const data = await response.json<{ course: { modules: unknown[]; finalQuestions: Array<Record<string, unknown>> } }>();
    expect(data.course.modules).toHaveLength(3);
    expect(data.course.finalQuestions[0]).not.toHaveProperty("answer");
  });
