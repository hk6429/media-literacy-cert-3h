(() => {
  ...
  function renderLogin(message = "") {
    ...
    root.innerHTML = `<form id="admin-login" class="stack-form"><h2>管理員登入</h2>${message ? `<p class="alert error">${escapeHtml(message)}</p>` : ""}<label for="password">管理密碼</label><input id="password" name="password" type="password" autocomplete="current-password" required><button class="button primary" type="submit">進入後臺</button></form>`;
  ...
  async function renderDashboard(query = "") {
    try {
      ...
      root.innerHTML = `<div class="admin-tools"><form id="record-search" class="search-form"><label for="q">搜尋完成教師</label><div><input id="q" name="q" value="${escapeHtml(query)}" placeholder="姓名、學校、Email、證書編號"><button class="button primary" type="submit">搜尋</button></div></form><div><a class="button secondary" href="/api/admin/export.csv">匯出 CSV</a><button id="logout" class="button ghost" type="button">登出</button></div></div>
        <div class="stats-grid"><article><span>已建立紀錄</span><strong>${stats.enrolled}</strong></article><article><span>已完成</span><strong>${stats.completed}</strong></article><article><span>平均總測驗</span><strong>${stats.averageFinalScore}</strong></article><article><span>累積學習</span><strong>${stats.totalActiveHours} 小時</strong></article></div>
        <div class="table-wrap"><table><caption>完成教師 ${recordsResult.total} 人</caption><thead><tr><th>姓名／學校</th><th>Email</th><th>學習分鐘</th><th>總測驗</th><th>完成時間</th><th>證書</th><th>微教案</th></tr></thead><tbody>${recordsResult.rows.length ? recordsResult.rows.map((row) => {
