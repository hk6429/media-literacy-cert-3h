  ...
  async function verify(code) {
    ...
    try {
      ...
      result.innerHTML = certificate.valid
        ? `<article class="verify-result valid"><p class="eyebrow">有效的本站完成證明</p><h2>${escapeHtml(certificate.maskedName)}</h2><p>${escapeHtml(certificate.school)}</p><p>完成日期：${new Intl.DateTimeFormat("zh-TW", { dateStyle: "long" }).format(new Date(certificate.completedAt))}</p><p>完成證明編號：${escapeHtml(code.trim().toUpperCase())}</p></article>`
        : `<p class="alert error">查無有效的本站完成證明。請核對編號是否完整。</p>`;
    } catch (error) {
